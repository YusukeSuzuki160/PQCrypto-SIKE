...
typedef struct {
    ...
    XNtt_forward_orig_uio_map maps[ MAX_UIO_MAPS ];
} XNtt_forward_orig_uio_info;

/***************** Variable Definitions **************************************/
static XNtt_forward_orig_uio_info uio_info;
...
static int uio_info_read_name(XNtt_forward_orig_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNtt_forward_orig_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNtt_forward_orig_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNtt_forward_orig_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNtt_forward_orig_Initialize(XNtt_forward_orig *InstancePtr, const char* InstanceName) {
	XNtt_forward_orig_uio_info *InfoPtr = &uio_info;
