...
#pragma HLS INTERFACE s_axilite port = return bundle = control
    ...
    {
        // 案C: 二乗(parr1*parr1)とz候補(zarr1*parr1)は同じparr1から計算され
        // 互いに依存しないため、あえて別々の呼び出し文に戻して2つの物理
        // インスタンスを生成させ、Vitisが並列スケジューリングしてくれるか
        // を検証する(1インスタンス共有版より高速化するが、LUT/DSPは
        // 増える方向のトレードオフ)。
        uint32_t parr2ref[MW_NWORDS];
        uint32_t zcand[MW_NWORDS];
        montMult_words(parr1, parr1, m, mprime, parr2ref); // 二乗
        montMult_words(zarr1, parr1, m, mprime, zcand);    // z候補(二乗と並列実行可能)
