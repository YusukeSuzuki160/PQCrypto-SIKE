...
#pragma HLS INTERFACE s_axilite port = return bundle = control
    ...
    // FromMontは孤立した1回だけの呼び出しで、ペアになる相手がいない
    // (直前のメインループが完了するまで計算できないため、事前変換の
    // 小ループへ混ぜることもできない)。それでも「小さなループで包む」
    // という形自体がVitisの共有判断に影響するかを検証するため、
    // 敢えてtrip count 1のループで包んでみる。
    uint32_t final_x[1][MW_NWORDS], final_y[1][MW_NWORDS], final_out[1][MW_NWORDS];
    for (unsigned k = 0; k < MW_NWORDS; k++) { final_x[0][k] = zarr1[k]; final_y[0][k] = one[k]; }
    for (unsigned s = 0; s < 1; s++)
    {
        montMult_words(final_x[s], final_y[s], m, mprime, final_out[s]);
    }
    for (unsigned k = 0; k < MW_NWORDS; k++) p[k] = final_out[0][k]; // p = FromMont(zarr1)
}
