#pragma once
// mont_ops_fios_csa_flat.hpp
// FIOS + Pre-sum — 外側ループ完全展開版
//
// = mont_ops_fios_csa.hpp (PIPELINE) + mont_ops_fios_flat.hpp (UNROLL) の組み合わせ
//
// 外側ループ PIPELINE (従来 FIOS-CSA):
//   II ≥ 依存深さ、各反復開始が均等に間隔を置く
//   7ns: 29 cycles (CSA の追加効果なし)
//
// 外側ループ UNROLL (本版 FIOS-CSA-Flat):
//   HLS が 4 反復すべてのデータ依存を直接追う
//   t[0] が書き込まれた直後に次反復の u_{i+1} 計算を開始できる
//   さらに CSA により carry chain = 5ns/step < 7ns → 1 cycle/step
//   → 反復間の「待ち時間」と carry chain 深さを同時に削減
//
// 想定効果 (7ns clock):
//   通常 FIOS PIPELINE: 29 cycles × 4.9 ns = 143.1 ns
//   本版:               ~22 cycles × 4.9 ns ≈ 108 ns  (目標)
//
// アルゴリズム:
//   FIOS-CSA と同じ。外側 FIOS_OUTER の pragma のみ UNROLL に変更。
...
#pragma HLS UNROLL
        ...
        // 外側ループを完全展開 (PIPELINE → UNROLL)
        // HLS が全 N 反復のデータフローを一括で把握し、最適スケジューリング
...
#pragma HLS UNROLL
            ...
            // ---- Step C: 逐次キャリーチェーン (2-input + 3-input add のみ, ≈5ns/step) ----
