...
struct PackedOps
{
    ...
    static constexpr unsigned MAX_ACC_LIMBS = 2*MAX_NWORDS + 9;
    ...
    #pragma HLS ARRAY_PARTITION variable=acc cyclic factor=2 dim=1
        ...
        #pragma HLS loop_tripcount min=1 max=MAX_NWORDS
            ...
            #pragma HLS ARRAY_PARTITION variable=part complete dim=1
                ...
                part[2] = (ap_uint<64>)prod.range(191,128);
                part[3] = (ap_uint<64>)prod.range(255,192);
                part[4] = (ap_uint<64>)prod.range(319,256);
                part[5] = (ap_uint<64>)prod.range(383,320);
                part[6] = (ap_uint<64>)prod.range(447,384);
                part[7] = (ap_uint<64>)prod.range(511,448);
                part[8] = 0;

                ap_uint<64> carry = 0;
                add_parts: for (unsigned k=0;k<9;++k){
                #pragma HLS UNROLL
                ...
                if (carry && limb + 9 < MAX_ACC_LIMBS) {
                    ap_uint<128> sum = (ap_uint<128>)acc[limb + 9] + (ap_uint<128>)carry;
                    acc[limb + 9] = (ap_uint<64>)sum;
                }
            }
        }
