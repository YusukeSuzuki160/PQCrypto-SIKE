...
struct PackedOps
{
    ...
    static constexpr unsigned MAX_ACC_LIMBS = 2*MAX_NWORDS + 5;
    ...
    #pragma HLS ARRAY_PARTITION variable=acc cyclic factor=2 dim=1
        ...
        #pragma HLS loop_tripcount min=1 max=MAX_NWORDS
            ...
            #pragma HLS ARRAY_PARTITION variable=part complete dim=1
                ...
                ap_uint<64> carry = 0;
                add_parts: for (unsigned k=0;k<5;++k){
                #pragma HLS UNROLL
                ...
                if (carry && limb + 5 < MAX_ACC_LIMBS) {
                    ap_uint<128> sum = (ap_uint<128>)acc[limb + 5] + (ap_uint<128>)carry;
                    acc[limb + 5] = (ap_uint<64>)sum;
                }
            }
        }
