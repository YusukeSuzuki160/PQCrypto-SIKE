# 1 "test_karatsuba_handwritten.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "test_karatsuba_handwritten.cpp" 2
...
# 4 "test_karatsuba_handwritten.cpp" 2
...
# 5 "test_karatsuba_handwritten.cpp" 2
...
# 6 "test_karatsuba_handwritten.cpp" 2
...
# 7 "test_karatsuba_handwritten.cpp" 2
...
void mp_mul(const digit_t *a, const digit_t *b, digit_t *c, const unsigned nwords)
{
  if (nwords <= 4)
  {
    VITIS_LOOP_18_1: for (unsigned i = 0; i < 2 * nwords; ++i)
      c[i] = 0;
    VITIS_LOOP_20_2: for (unsigned i = 0; i < nwords; ++i)
      VITIS_LOOP_21_3: for (unsigned j = 0; j < nwords; ++j)
        c[i + j] += a[i] * b[j];
    return;
  }
  ...
  digit_t z1[16];
  digit_t z2[16];
  VITIS_LOOP_29_4: for (unsigned i = 0; i < 16; ++i)
  {
    z0[i] = 0;
    z1[i] = 0;
    z2[i] = 0;
  ...
  VITIS_LOOP_40_5: for (unsigned i = 0; i < 2 * nwords; ++i)
    c[i] = z0[i] ^ z1[i] ^ z2[i];
}

__attribute__((sdx_kernel("test_mul_karatsuba_handwritten", 0))) void test_mul_karatsuba_handwritten(const digit_t a[8], const digit_t b[8], digit_t c[16],
                                    const unsigned )
{
#line 12 "/home/meltpoint/eeic/PQCrypto-SIKE/mp_mul_rewriter/auto/run_karatsuba_hls_compare.tcl"
#pragma HLSDIRECTIVE TOP name=test_mul_karatsuba_handwritten
# 46 "test_karatsuba_handwritten.cpp"
