...
// 多倍長乗算関数（学校式）
void mp_mul_16bit(const uint64_t *a, const uint64_t *b, uint64_t *c, const unsigned int nwords)
{
    // Auto-converted by mpx_auto_rewriter_v4_2 (schoolbook multiplication)
    constexpr unsigned MAX_NWORDS = 8;
    constexpr unsigned MUL_BITS = 16;
...
int main()
{
    ...
    mp_mul_16bit(a, b, c, 2);

    std::cout << "Test: 16-bit schoolbook multiplication\n";
