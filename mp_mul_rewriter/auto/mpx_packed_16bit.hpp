...
#pragma HLS loop_tripcount min = 1 max = 16
            ...
            // Pairwise 16-bit × 16-bit schoolbook accumulation
            for (unsigned i = 0; i < nwords; ++i)
            {
#pragma HLS loop_tripcount min = 1 max = 8
#pragma HLS PIPELINE II = 1
                ap_uint<16> a16 = (ap_uint<16>)a[i];

                for (unsigned j = 0; j < nwords; ++j)
                {
#pragma HLS UNROLL
                    ap_uint<16> b16 = (ap_uint<16>)b[j];

                    // 16x16 -> 32 product
                    ap_uint<32> prod = a16 * b16;
                    ap_uint<16> limb0 = prod.range(15, 0);
                    ap_uint<16> limb1 = prod.range(31, 16);

                    unsigned pos = i + j;

                    // Add into c[pos .. pos+1] with carry propagation
                    ap_uint<16> limbs[2];
                    limbs[0] = limb0;
                    limbs[1] = limb1;
                    ...
                    for (int k = 0; k < 2; ++k)
                    {
                        if (pos + (unsigned)k >= 2 * nwords)
                            break;
                        ap_uint<32> sum = (ap_uint<32>)c[pos + (unsigned)k] + limbs[k] + (ap_uint<32>)carry;
                        c[pos + (unsigned)k] = (Digit)sum.range(15, 0);
                        carry = (ap_uint<16>)sum.range(31, 16);
                    }
                    unsigned k = 2;
                    while (carry && pos + k < 2 * nwords)
                    {
                        ap_uint<32> sum = (ap_uint<32>)c[pos + k] + (ap_uint<32>)carry;
                        c[pos + k] = (Digit)sum.range(15, 0);
                        carry = (ap_uint<16>)sum.range(31, 16);
                        ++k;
                    }
                }
