# 1 "../test_karatsuba_handwritten_flat.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../test_karatsuba_handwritten_flat.cpp" 2
...
# 3 "../test_karatsuba_handwritten_flat.cpp" 2
...
# 4 "../test_karatsuba_handwritten_flat.cpp" 2
...
# 5 "../test_karatsuba_handwritten_flat.cpp" 2
...
#pragma HLS INLINE
 Big4 A = 0, B = 0;
  VITIS_LOOP_19_1: for (unsigned i = 0; i < nwords; ++i)
  {
#pragma HLS PIPELINE II = 1
 A.range((i + 1) * W - 1, i * W) = a[i];
  ...
  VITIS_LOOP_26_2: for (unsigned i = 0; i < 2 * nwords; ++i)
  {
#pragma HLS PIPELINE II = 1
...
static unsigned add_limbs(const digit_t *xa, unsigned na, const digit_t *xb, unsigned nb, digit_t *dst)
{
  ...
  ap_uint<W + 1> carry = 0;
  VITIS_LOOP_37_1: for (unsigned i = 0; i < n; ++i)
  {
#pragma HLS PIPELINE II = 1
...
#pragma HLS INLINE off
 digit_t Al[N_LO], Ah[N_LO], Bl[N_LO], Bh[N_LO];
  VITIS_LOOP_54_1: for (unsigned i = 0; i < N_LO; ++i)
  {
#pragma HLS PIPELINE II = 1
 Al[i] = a[i];
    Bl[i] = b[i];
  ...
  VITIS_LOOP_65_2: for (unsigned i = 0; i < 2 * N_LO; ++i)
  {
    z0[i] = 0;
    z2[i] = 0;
  }
  VITIS_LOOP_70_3: for (unsigned i = 0; i < 2 * (N_LO + 2); ++i)
    z1[i] = 0;
  VITIS_LOOP_72_4: for (unsigned i = 0; i < N_LO + 2; ++i)
  {
    Sal[i] = 0;
    Sbl[i] = 0;
  }
  ...
  VITIS_LOOP_86_5: for (unsigned i = 0; i < 2 * N_LO; ++i)
  {
#pragma HLS PIPELINE II = 1
 W0 += (ap_int<2 * W * N + 32>)(ap_uint<W>)z0[i] << (int)(i * W);
    W2 += (ap_int<2 * W * N + 32>)(ap_uint<W>)z2[i] << (int)(i * W);
  }
  VITIS_LOOP_92_6: for (unsigned i = 0; i < 2 * n_mul; ++i)
  {
#pragma HLS PIPELINE II = 1
 W1v += (ap_int<2 * W * N + 32>)(ap_uint<W>)z1[i] << (int)(i * W);
  ...
  Big16 out = 0;
  VITIS_LOOP_103_7: for (unsigned i = 0; i < 2 * N; ++i)
  {
#pragma HLS PIPELINE II = 1
 ap_int<W + 2> limb = Pacc.range((int)((i + 1) * W - 1), (int)(i * W));
    out.range((i + 1) * W - 1, i * W) = (ap_uint<W>)limb;
  }
  VITIS_LOOP_109_8: for (unsigned i = 0; i < 2 * N; ++i)
  {
#pragma HLS PIPELINE II = 1
...
__attribute__((sdx_kernel("test_mul_karatsuba_handwritten_flat", 0))) void test_mul_karatsuba_handwritten_flat(const digit_t a[8], const digit_t b[8], digit_t c[16])
{
#line 8 "/home/meltpoint/eeic/PQCrypto-SIKE/mp_mul_rewriter/auto/run_kara_sweep_flat.tcl"
#pragma HLSDIRECTIVE TOP name=test_mul_karatsuba_handwritten_flat
# 117 "../test_karatsuba_handwritten_flat.cpp"
