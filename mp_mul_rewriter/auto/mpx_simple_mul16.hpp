...
template <unsigned N>
void mul(const ap_uint<16> a[N], const ap_uint<16> b[N],
         ap_uint<32> c[2*N])
{
#pragma HLS INLINE off
    ...
    #pragma HLS LOOP_TRIPCOUNT min=1 max=N
        ap_uint<32+1> carry = 0;

        inner: for (unsigned j = 0; j < N; ++j) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=N
            ap_uint<32> prod = (ap_uint<32>)a[i] * (ap_uint<32>)b[j];
            ap_uint<32+1> sum = (ap_uint<32+1>)c[i+j] + prod + carry;
            c[i+j] = (ap_uint<32>)sum;
            carry = sum >> 32;
        }
        ...
        unsigned k = i + N;
        while (carry != 0) {
            ap_uint<32+1> sum = (ap_uint<32+1>)c[k] + carry;
            c[k] = (ap_uint<32>)sum;
            carry = sum >> 32;
            ++k;
        }
