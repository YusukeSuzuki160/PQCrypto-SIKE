...
static std::string stripCommentsAndStrings(const std::string& in) {
  ...
  enum State { CODE, SLASH, LINE_CMT, BLOCK_CMT, SQUOTE, DQUOTE, ESC_S, ESC_D } st = CODE;
  for (char c : in) {
    switch (st) {
      case CODE:
        if (c == '/') { st = SLASH; out.push_back(' '); }
        else if (c == '\'') { st = SQUOTE; out.push_back(' '); }
        else if (c == '"')  { st = DQUOTE; out.push_back(' '); }
        else out.push_back(c);
        break;
      case SLASH:
        if (c == '/') { st = LINE_CMT; out.back() = ' '; }
        else if (c == '*') { st = BLOCK_CMT; out.back() = ' '; }
        else { st = CODE; out.push_back(c); }
        break;
      case LINE_CMT:
        if (c == '\n') { st = CODE; out.push_back('\n'); }
        else out.push_back(' ');
        break;
      case BLOCK_CMT:
        if (c == '*') st = SLASH; // temp reuse
        out.push_back(' ');
        break;
      case SQUOTE:
        if (c == '\\') st = ESC_S;
      ...
      case DQUOTE:
        ...
        out.push_back(' ');
        break;
      case ESC_S: st = SQUOTE; out.push_back(' '); break;
      case ESC_D: st = DQUOTE; out.push_back(' '); break;
    }
  }
  return out;
}
...
static unsigned countMacroHitsInBody(const CompoundStmt* Body,
  ...
  for (const auto& name : names) {
    ...
    for (size_t pos = 0;;) {
      ...
      while (p < txt.size() && std::isspace((unsigned char)txt[p])) ++p;
      if (okid && p < txt.size() && txt[p] == '(') ++hits;
      pos += name.size();
    }
  }
  return hits;
...
static std::string extractHLSPragmas(const CompoundStmt* Body,
  ...
  while (std::getline(in, line)) {
    ...
    if (s.rfind("#pragma HLS", 0) == 0) out += line + "\n";
  }
  return out;
}
...
static bool foldInt64(const llvm::APInt& api, long long& out) {
  if (api.getBitWidth() > 63) return false;
...
static bool buildLinFormRec(const Expr* E,
  ...
  if (!E) { L.ok=false; return false; }
  E = peel(E);

  if (const auto* IL = dyn_cast<IntegerLiteral>(E)) {
    long long v = 0;
    if (!foldInt64(IL->getValue(), v)) { L.ok=false; return false; }
    L.c0 += sgn * v; return true;
  }
  if (const auto* DRE = dyn_cast<DeclRefExpr>(E)) {
    const ValueDecl* VD = DRE->getDecl();
    if (VD == I) { L.ci += sgn; return true; }
    if (VD == J) { L.cj += sgn; return true; }
    if (VD == N) { L.cn += sgn; return true; }
    L.ok = false; return false;
  }
  if (const auto* UO = dyn_cast<UnaryOperator>(E)) {
  ...
  if (const auto* BO = dyn_cast<BinaryOperator>(E)) {
    switch (BO->getOpcode()) {
      ...
      case BO_Mul: {
        ...
        if (!K || !V) { K = dyn_cast<IntegerLiteral>(Rhs); V = dyn_cast<DeclRefExpr>(Lhs); }
        if (!K || !V) { L.ok=false; return false; }
        long long k=0; if (!foldInt64(K->getValue(), k)) { L.ok=false; return false; }
        const ValueDecl* VD = V->getDecl();
        if (VD == I) L.ci += sgn * k;
        else if (VD == J) L.cj += sgn * k;
        else if (VD == N) L.cn += sgn * k;
        else { L.ok=false; return false; }
        return true;
      }
      default: break;
    }
...
static bool isN_or_2N(const Expr* Arg, const VarDecl* I, const VarDecl* J, const ParmVarDecl* N) {
  LinForm L;
  if (!buildLinFormRec(Arg, I, J, N, +1, L) || !L.ok) return false;
...
static bool indexMatchesAllowedShapesCore(const Expr* Eidx,
                                          ...
                                          const ParmVarDecl* N) {
  LinForm L;
  if (!buildLinFormRec(Eidx, I, J, N, +1, L) || !L.ok) return false;

  // 1) 低位: i or i+j (+小定数)
  if (L.cn == 0) {
    if (L.ci == 1 && (L.cj == 0 || L.cj == 1)) return true;
    if (std::llabs(L.c0) <= 4 && L.ci == 1 && (L.cj == 0 || L.cj == 1)) return true;
  }

  // 2) MSW 系:
  //   2-1) 典型: n-1-i, n-1-(i+j), 2n-1-i (+小定数)
  if ((L.cn == 1 || L.cn == 2)) {
    if ((L.ci == -1) && (L.cj == 0 || L.cj == -1) && std::llabs(L.c0 + 1) <= 4) return true;
    //   2-2) 最終語の定数: n-1, 2n-1 （ci=cj=0, c0≈-1）
    if (L.ci == 0 && L.cj == 0 && std::llabs(L.c0 + 1) <= 4) return true;
  }
  return false;
}
...
static bool indexMatchesAllowedShapes(const Expr* Idx,
  ...
  // 呼び出し名の確認
  std::string name;
  if (!getDirectCallName(Idx, name)) return false;
...
public:
  ...
  bool isArrayFromParamCore(const Expr* E, const ParmVarDecl* P) const {
    ...
    if (const auto* ASE = dyn_cast<ArraySubscriptExpr>(E)) {
      ...
      if (const auto* DRE = dyn_cast<DeclRefExpr>(base)) return DRE->getDecl() == P;
    }
    // *(a + k) / *(k + a)
    if (const auto* UO = dyn_cast<UnaryOperator>(E)) {
        ...
        if (const auto* BO = dyn_cast<BinaryOperator>(S)) {
          if (BO->getOpcode() == BO_Add) {
            auto isBase = [&](const Expr* X){
              X = peel(X);
              if (const auto* DRE = dyn_cast<DeclRefExpr>(X)) return DRE->getDecl() == P;
              return false;
            };
            if (isBase(BO->getLHS()) || isBase(BO->getRHS())) return true;
          }
        }
      }
    }
  ...
  bool isAliasOf(const Expr* E, const ParmVarDecl* P) const {
    ...
    if (const auto* DRE = dyn_cast<DeclRefExpr>(E)) {
      if (const auto* VD = dyn_cast<VarDecl>(DRE->getDecl())) {
        if (P == A) return AliasA.count(VD);
        if (P == B) return AliasB.count(VD);
      }
    }
    return false;
  }
  ...
  bool VisitVarDecl(VarDecl* VD) {
    if (const Expr* Init = VD->getInit()) {
      if (isArrayFromParamCore(Init, A)) AliasA.insert(VD);
      else if (isArrayFromParamCore(Init, B)) AliasB.insert(VD);
    }
    return true;
  }
  ...
  bool VisitBinaryOperator(BinaryOperator* BO) {
    ...
    if (BO->isAssignmentOp() && BO->getOpcode() == BO_Assign) {
      ...
      if (const auto* DRE = dyn_cast<DeclRefExpr>(LHS)) {
        if (const auto* VD = dyn_cast<VarDecl>(DRE->getDecl())) {
          if (isArrayFromParamCore(RHS, A)) AliasA.insert(VD);
          if (isArrayFromParamCore(RHS, B)) AliasB.insert(VD);
        }
      }
    }
    ...
    if (BO->getOpcode() == BO_Mul) {
      ...
      if (hit) {
        ++MulHitsAST;
        if (TraceOpt) traceMul("BO_Mul", L, R);
      }
    }
    ...
    if (ForDepth >= 2) {
      if (IgnoreOpsInMacros && BO->getExprLoc().isMacroID()) {
        ...
        bool bad = false;
        if (BanMod     && K == BO_Rem) bad = true;                  // %
        if (BanDiv     && K == BO_Div) bad = true;                  // /
        if (BanShift   && (K == BO_Shl || K == BO_Shr)) bad = true; // << >>
        if (BanBitwise && (K == BO_And || K == BO_Or || K == BO_Xor)) bad = true; // &,|,^
        if (bad) ++ForbiddenOps;
      }
    }
    ...
    if (BO->isAssignmentOp()) {
      ...
      if (const auto* ASE = dyn_cast<ArraySubscriptExpr>(L)) {
        ...
        if (containsNwordsMinus(ASE->getIdx())) MSWStyle = true;
      }
    }
    return true;
  }
  ...
  bool VisitCXXOperatorCallExpr(CXXOperatorCallExpr* CE) {
    if (CE->getOperator() != OO_Star || CE->getNumArgs() < 2) return true;
    ...
    if (hit) {
      ++MulHitsAST;
      if (TraceOpt) traceMul("OO_Star", L, R);
    }
    return true;
  }
  ...
  bool VisitCallExpr(CallExpr* CE) {
    if (ForDepth < 2 || !ForbidInnerCalls) return true;
    if (IgnoreCallsInMacros && CE->getExprLoc().isMacroID()) return true;

    const FunctionDecl* FD = CE->getDirectCallee();
    if (!FD) { ++InnerCallsDisallowed; return true; }
    std::string name = FD->getNameAsString();
    if (AllowSet.count(name) == 0) ++InnerCallsDisallowed;
    return true;
  }
  ...
  bool TraverseForStmt(ForStmt* FS) {
    ForDepth++;
    if (ForDepth >= 2) HasNested = true;

    const VarDecl* LV = guessLoopVar(FS);
    if (LV) LoopVars.push_back(LV);

    RecursiveASTVisitor<BodyScanner>::TraverseForStmt(FS);

    if (LV && !LoopVars.empty() && LoopVars.back() == LV) LoopVars.pop_back();
    ForDepth--;
    return true;
  }
  ...
  void postCountMacros() {
    ...
    if (names.empty()) names.push_back("MUL"); // 既定
...
private:
  void traceMul(const char* kind, const Expr* L, const Expr* R) const {
    ...
    if (ls.size() > 120) ls = ls.substr(0, 120) + "...";
    if (rs.size() > 120) rs = rs.substr(0, 120) + "...";
  ...
  bool containsNwordsMinus(const Expr* E) const {
    ...
    std::function<void(const Expr*)> walk = [&](const Expr* X){
      X = peel(X);
      if (const auto* DRE = dyn_cast<DeclRefExpr>(X)) if (DRE->getDecl() == N) hasN = true;
      if (const auto* BO = dyn_cast<BinaryOperator>(X)) {
        if (BO->getOpcode() == BO_Sub) hasMinus = true;
        walk(BO->getLHS()); walk(BO->getRHS());
...
public:
  ...
  void run(const MatchFinder::MatchResult& Res) override {
    ...
    for (const auto& s : AllowCallInner) allowSet.insert(s);
    ...
    auto nameOf = [](const ParmVarDecl* P)->std::string { return P ? P->getNameAsString() : ""; };
    std::string hlsPragmas = extractHLSPragmas(Body, R.getSourceMgr(), Res.Context->getLangOpts());

    std::ostringstream oss;
    oss << "{\n";
    if (!hlsPragmas.empty()) oss << "  " << hlsPragmas;
