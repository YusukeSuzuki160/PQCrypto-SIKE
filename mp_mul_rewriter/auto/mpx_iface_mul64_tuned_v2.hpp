...
struct PackedOps
{
    ...
    static_assert(W == 64, "This tuned header expects Digit to be 64-bit Digit.");
    ...
    #pragma HLS ARRAY_PARTITION variable=S_lo cyclic factor=2 dim=1
    #pragma HLS ARRAY_PARTITION variable=S_hi cyclic factor=2 dim=1
        ...
        #pragma HLS loop_tripcount min=1 max=MAX_NWORDS
            ...
            #pragma HLS RESOURCE variable=prod core=DSP48
                ...
                unsigned k0 = i + j;
                S_lo[k0]     = (Col64Sum)S_lo[k0]     + (Col64Sum)prod.range(63, 0);
                S_hi[k0 + 1] = (Col64Sum)S_hi[k0 + 1] + (Col64Sum)prod.range(127, 64);
            }
        }
