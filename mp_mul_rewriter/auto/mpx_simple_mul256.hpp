...
template <unsigned N>
void mul(const ap_uint<256> a[N], const ap_uint<256> b[N],
         ap_uint<512> c[2*N])
{
#pragma HLS INLINE off
    ...
    #pragma HLS LOOP_TRIPCOUNT min=1 max=N
        ap_uint<512+1> carry = 0;

        inner: for (unsigned j = 0; j < N; ++j) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=N
            ap_uint<512> prod = (ap_uint<512>)a[i] * (ap_uint<512>)b[j];
            ap_uint<512+1> sum = (ap_uint<512+1>)c[i+j] + prod + carry;
            c[i+j] = (ap_uint<512>)sum;
            carry = sum >> 512;
        }
        ...
        unsigned k = i + N;
        while (carry != 0) {
            ap_uint<512+1> sum = (ap_uint<512+1>)c[k] + carry;
            c[k] = (ap_uint<512>)sum;
            carry = sum >> 512;
            ++k;
        }
