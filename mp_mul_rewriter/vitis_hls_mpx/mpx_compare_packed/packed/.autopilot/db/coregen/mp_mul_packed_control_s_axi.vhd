...
entity mp_mul_packed_control_s_axi is
generic (
...
end entity mp_mul_packed_control_s_axi;
...
architecture behave of mp_mul_packed_control_s_axi is
    ...
    constant ADDR_NWORDS_DATA_0 : INTEGER := 16#34#;
    constant ADDR_NWORDS_CTRL   : INTEGER := 16#38#;
    ...
    signal int_nwords          : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NWORDS_DATA_0 =>
                        rdata_data <= RESIZE(int_nwords(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    nwords               <= STD_LOGIC_VECTOR(int_nwords);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_nwords(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NWORDS_DATA_0) then
                    int_nwords(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_nwords(31 downto 0));
                end if;
            end if;
        end if;
    end process;
