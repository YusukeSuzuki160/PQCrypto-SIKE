...
typedef struct {
    ...
    XMp_mul_packed_uio_map maps[ MAX_UIO_MAPS ];
} XMp_mul_packed_uio_info;

/***************** Variable Definitions **************************************/
static XMp_mul_packed_uio_info uio_info;
...
static int uio_info_read_name(XMp_mul_packed_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMp_mul_packed_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMp_mul_packed_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMp_mul_packed_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMp_mul_packed_Initialize(XMp_mul_packed *InstancePtr, const char* InstanceName) {
	XMp_mul_packed_uio_info *InfoPtr = &uio_info;
