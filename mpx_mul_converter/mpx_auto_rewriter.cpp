...
static llvm::cl::opt<std::string> PackedOpsType(
    "packed-ops-type", llvm::cl::desc(
        "Type of PackedOps to use (default, schoolbook-16, schoolbook-32, schoolbook-64, schoolbook-128, schoolbook-256, karatsuba)"),
    llvm::cl::init("default"), llvm::cl::cat(Cat));

static llvm::cl::list<std::string> KaratsubaCallee(
    "karatsuba-callee",
    llvm::cl::desc("Callee names for Karatsuba half-size multiply detection (repeatable; default mp_mul)"),
    llvm::cl::ZeroOrMore, llvm::cl::cat(Cat));

static llvm::cl::opt<unsigned> KaratsubaMinHalfCalls(
    "karatsuba-min-half-calls",
    llvm::cl::desc("Minimum half-sized (nwords/2 or (nwords+1)/2) callee invocations for Karatsuba detection"),
    llvm::cl::init(3), llvm::cl::cat(Cat));

static llvm::cl::opt<unsigned> KaraBaseWords(
    "kara-base-words",
    llvm::cl::desc("Karatsuba: word count for direct-mul base (MPX_KARA_BASE_WORDS; 0=header default 4)"),
    llvm::cl::init(0), llvm::cl::cat(Cat));

static llvm::cl::opt<unsigned> KaraMulBits(
    "kara-mul-bits",
    llvm::cl::desc("Karatsuba: direct multiplier width in bits (MPX_MUL_BITS; 0=auto W*base)"),
    llvm::cl::init(0), llvm::cl::cat(Cat));
...
// ---- Karatsuba 検出: 半長 nwords への callee / 自己再帰呼び出し ----
static const Expr *peelCastsForHalf(const Expr *E)
{
  if (!E)
    return nullptr;
  E = peel(E);
  if (const auto *ICE = dyn_cast<ImplicitCastExpr>(E))
    return peelCastsForHalf(ICE->getSubExpr());
  if (const auto *CSE = dyn_cast<CStyleCastExpr>(E))
    return peelCastsForHalf(CSE->getSubExpr());
  return E;
}

static bool exprRefsNwordsParm(const Expr *E, const ParmVarDecl *N)
{
  E = peelCastsForHalf(E);
  if (const auto *DRE = dyn_cast<DeclRefExpr>(E))
    return DRE->getDecl() == N;
  return false;
}

static bool intLiteralEquals(const Expr *E, uint64_t v)
{
  const auto *IL = dyn_cast<IntegerLiteral>(peelCastsForHalf(E));
  if (!IL)
    return false;
  return IL->getValue().getZExtValue() == v;
}

static bool isHalfNwordsSize(const Expr *E, const ParmVarDecl *N)
{
  E = peelCastsForHalf(E);
  const auto *BO = dyn_cast<BinaryOperator>(E);
  if (!BO)
    return false;
  const BinaryOperatorKind K = BO->getOpcode();
  const bool isDiv = (K == BO_Div);
  const bool isShr = (K == BO_Shr);

  if (isDiv && intLiteralEquals(BO->getRHS(), 2))
  {
    const Expr *L = peelCastsForHalf(BO->getLHS());
    if (exprRefsNwordsParm(L, N))
      return true;
    if (const auto *A = dyn_cast<BinaryOperator>(L))
      if (A->getOpcode() == BO_Add && exprRefsNwordsParm(A->getLHS(), N) &&
          intLiteralEquals(A->getRHS(), 1))
        return true;
  }
  if (isShr && exprRefsNwordsParm(BO->getLHS(), N) && intLiteralEquals(BO->getRHS(), 1))
    return true;
  return false;
}

class KaratsubaShapeScanner : public RecursiveASTVisitor<KaratsubaShapeScanner>
{
public:
  KaratsubaShapeScanner(const FunctionDecl *Self, const ParmVarDecl *Nwords,
                        const std::unordered_set<std::string> &calleeNames,
                        unsigned minHalf)
      : SelfCanon(Self->getCanonicalDecl()), N(Nwords), Names(calleeNames),
        MinHalf(minHalf) {}
  ...
  bool VisitCallExpr(CallExpr *CE)
  {
    ...
    const bool selfRec = (CalleeCanon == SelfCanon);
    const std::string nm = Callee->getNameAsString();
    const bool named = Names.count(nm) > 0;
    if (!named && !selfRec)
      return true;
    if (CE->getNumArgs() < 4)
      return true;
    if (isHalfNwordsSize(CE->getArg(3), N))
      ++HalfCalls;
    return true;
  }

  bool isCandidate() const { return HalfCalls >= MinHalf; }
  unsigned getHalfCalls() const { return HalfCalls; }
...
public:
  ...
  void run(const MatchFinder::MatchResult &Res) override
  {
    ...
    const bool useKaratsuba = (PackedOpsType.getValue() == "karatsuba");
    ...
    if (useKaratsuba)
    {
      ...
      if (KaratsubaCallee.size() == 0)
        calleeSet.insert("mp_mul");
      else
        for (const auto &s : KaratsubaCallee)
          calleeSet.insert(s);
      KaratsubaShapeScanner ks(FD, N, calleeSet, KaratsubaMinHalfCalls);
      ks.TraverseStmt(const_cast<CompoundStmt *>(Body));
      karaHalf = ks.getHalfCalls();
      candidate = digitsOK && ks.isCandidate();
    }
    else
    {
      candidate =
    ...
    llvm::outs() << "[mpx-auto] Function '" << FD->getNameAsString()
                 << "': packed_ops=" << PackedOpsType.getValue();
    if (useKaratsuba)
    {
      llvm::outs() << ", kara_half_calls=" << karaHalf
                   << ", kara_min=" << KaratsubaMinHalfCalls.getValue();
    }
    else
    {
      llvm::outs() << ", mulHits=" << mulHits
    ...
    llvm::outs() << ", digitsOK=" << (digitsOK ? "1" : "0")
    ...
    const bool isKara = (PackedOpsType.getValue() == "karatsuba");

    auto karaConfigPreamble = []() -> std::string {
      std::ostringstream kh;
      if (KaraBaseWords.getValue() > 0)
        kh << "#ifndef MPX_KARA_BASE_WORDS\n#define MPX_KARA_BASE_WORDS "
           << KaraBaseWords.getValue() << "\n#endif\n";
      if (KaraMulBits.getValue() > 0)
        kh << "#ifndef MPX_MUL_BITS\n#define MPX_MUL_BITS "
           << KaraMulBits.getValue() << "\n#endif\n";
      return kh.str();
    };
    ...
    if (!IncludesInserted)
    {
      ...
      {
        ih << karaConfigPreamble();
        ih << "#include \"mpx_packed_karatsuba.hpp\"\n";
        R.InsertText(FileStart, ih.str(), true, true);
        IncludesInserted = true;
      }
    }
    ...
    else if (PackedOpsType.getValue() == "karatsuba")
    {
      if (KaraBaseWords.getValue() > 0)
        oss << "  // MPX_KARA_BASE_WORDS=" << KaraBaseWords.getValue() << " (see file header)\n";
      if (KaraMulBits.getValue() > 0)
        oss << "  // MPX_MUL_BITS=" << KaraMulBits.getValue() << " (see file header)\n";
    }
    else
    {
