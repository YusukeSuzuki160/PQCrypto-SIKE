// mpx_packed_32bit.hpp
// 32ビット学校式乗算用のPackedOpsクラス
...
#pragma HLS loop_tripcount min = 1 max = 16
            ...
            // Pairwise 32-bit × 32-bit schoolbook accumulation
            for (unsigned i = 0; i < nwords; ++i)
            {
#pragma HLS loop_tripcount min = 1 max = 8
#pragma HLS PIPELINE II = 1
                ap_uint<32> a32 = (ap_uint<32>)a[i];

                for (unsigned j = 0; j < nwords; ++j)
                {
#pragma HLS UNROLL
                    ap_uint<32> b32 = (ap_uint<32>)b[j];

                    // 32x32 -> 64 product
                    ap_uint<64> prod = a32 * b32;
