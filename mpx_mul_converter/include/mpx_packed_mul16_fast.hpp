...
struct PackedOps
{
    ...
    static constexpr unsigned BLOCK_BITS = 16;
    static constexpr unsigned MAX_ACC_LIMBS = 2*MAX_NWORDS + 2;
    ...
    #pragma HLS ARRAY_PARTITION variable=acc cyclic factor=2 dim=1
        ...
        #pragma HLS loop_tripcount min=1 max=MAX_NWORDS
            ...
            #pragma HLS PIPELINE II=1
                ...
                ap_uint<32> prod = (ap_uint<32>)ai * (ap_uint<32>)bj;
            ...
            #pragma HLS ARRAY_PARTITION variable=part complete dim=1
                ...
                ap_uint<64> carry = 0;
                add_parts: for (unsigned k=0;k<2;++k){
                #pragma HLS UNROLL
                ...
                if (carry && limb + 2 < MAX_ACC_LIMBS) {
                    ap_uint<128> sum = (ap_uint<128>)acc[limb + 2] + (ap_uint<128>)carry;
                    acc[limb + 2] = (ap_uint<64>)sum;
                }
            }
        }
