...
run_hls_one() {
  ...
  local tcl="$BUILD/run_kara_sweep_${tag}.tcl"
  cat > "$tcl" <<EOF
open_project -reset ${proj}
set_top ${top}
add_files ${src}
add_files ${INC}/mpx_packed_karatsuba.hpp
...
EOF
  ...
  if vitis_hls -f "$tcl" > "$OUT/hls_kara_${tag}.log" 2>&1; then
    local rpt="${BUILD}/${proj}/solution1/syn/report/${top}_csynth.rpt"
    if [[ -f "$rpt" ]]; then
...
for entry in "${SWEEPS[@]}"; do
  ...
  cpp="$EX/test_kara_sweep_${tag}.cpp"
  gen_config_cpp "$base" "$mul" "$cpp"
  run_hls_one "$tag" "$cpp" "test_mul_karatsuba_configurable" || true
done

run_hls_one "flat" "$EX/test_karatsuba_handwritten_flat.cpp" "test_mul_karatsuba_handwritten_flat" || true
