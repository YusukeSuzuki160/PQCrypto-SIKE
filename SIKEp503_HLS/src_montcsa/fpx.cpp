...
#include "mont_ops_fios_csa_square.hpp"
/********************************************************************************************
...
// copy_words((digit_t*)point_proj_t...) は構造体境界を超える型安全でない
// ポインタキャストだった(詳細はsrc/fpx.cppのコメント参照)。point_proj_t
// のコピーには copy_point (ec_isogeny.cpp, fpcopyをフィールド毎に適用)を
// 使う。copy_words 自体は本プロジェクトでは不要になったため元の単純な
// 実装のまま残す。
...
#pragma HLS DATAFLOW off
  #pragma HLS INLINE off
  // Auto-converted by montgomery_auto_rewriter, さらに手動で平方専用コア
  // (mont_ops_fios_csa_square.hpp, 別セッションでN=8合成しLUTを82.5%削減
  // 確認済み)へ差し替え。fpmul_mont(汎用mul)とは別ハードウェアとして
  // 全体で1個共有される(下記set_directive_allocationで強制)。
  constexpr unsigned MAX_NWORDS = 8;
  using Digit = std::remove_cv_t<std::remove_pointer_t<decltype(mc)>>;
