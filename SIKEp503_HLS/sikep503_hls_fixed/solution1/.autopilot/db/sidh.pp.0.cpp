...
int EphemeralKeyGeneration_A(const unsigned char *PrivateKeyA, unsigned char *PublicKeyA)
{
    ...
    VITIS_LOOP_141_1: for (row = 1; row < 125; row++)
    {
        ...
        VITIS_LOOP_175_3: for (i = 0; i < 7; i++)
        {
            point_proj_t pt_new;
...
int EphemeralKeyGeneration_B(const unsigned char *PrivateKeyB, unsigned char *PublicKeyB)
{
    ...
    index = 0;
    VITIS_LOOP_243_1: for (row = 1; row < 159; row++)
    {
        VITIS_LOOP_245_2: for (int j = 0; j < 159 - row; j++)
        {


            bool cond = (index < 159 - row);
            digit_t mask = 0 - (digit_t)cond;
        ...
        VITIS_LOOP_271_3: for (i = 0; i < 8; i++)
        {
            point_proj_t pt_new;
            copy_point(pts[i], pt_new);
...
# 329 "src/sidh.cpp"
    ...
    index = 0;
    VITIS_LOOP_345_1: for (row = 1; row < 125; row++)
    {
        VITIS_LOOP_347_2: for (int j = 0; j < 125 - row; j++)
        {
        ...
        VITIS_LOOP_370_3: for (i = 0; i < 7; i++)
        {
            point_proj_t pt_new;
            copy_point(pts[i], pt_new);
            eval_4_isog(pt_new, coeff);
...
int EphemeralSecretAgreement_B(const unsigned char *PrivateKeyB, const unsigned char *PublicKeyA, unsigned char *SharedSecretB)
{
    ...
    index = 0;
    VITIS_LOOP_427_1: for (row = 1; row < 159; row++)
    {
        VITIS_LOOP_429_2: for (int j = 0; j < 159 - row; j++)
        {
        ...
        VITIS_LOOP_452_3: for (i = 0; i < 8; i++)
        {
            point_proj_t pt_new;
            copy_point(pts[i], pt_new);
            eval_3_isog(pt_new, coeff);
