...
reg [31 : 0] ct_OPERATE_DEPTH = 0;
parameter ct_c_bitwidth = 64;
...
parameter ct_data_in_addr = 16;
parameter pk_data_in_addr = 28;
parameter ss_data_in_addr = 40;
...
reg  BREADY_reg = 0;
reg [ct_c_bitwidth - 1 : 0] mem_ct [ct_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_ct [ (ct_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * ct_DEPTH -1 : 0] = '{default : 'hz};
reg ct_write_data_finish;
...
reg write_ct_run_flag = 0;
reg write_one_ct_data_done = 0;
//write pk reg
reg [31 : 0] write_pk_count = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_ct_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (ct_c_bitwidth, ct_DEPTH, ct_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_ct_run_flag <= 1; 
        end
        else if ((write_one_ct_data_done == 1 && write_ct_count == ct_diff_count - 1) || ct_diff_count == 0) begin
            write_ct_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_ct_data_done === 1) begin
            write_ct_count = write_ct_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ct_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            ct_write_data_finish <= 0;
        end
        if (write_ct_run_flag == 1 && write_ct_count == ct_diff_count) begin
            ct_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_ct
    ...
    integer j;
    integer k;
    reg [31 : 0] ct_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = ct_c_bitwidth;
    ...
    while (1) begin
        ...
        for (k = 0; k < ct_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (ct_c_bitwidth < 32) begin
                    ct_data_tmp_reg = mem_ct[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < ct_c_bitwidth) begin
                            ct_data_tmp_reg[j] = mem_ct[k][i*32 + j];
                        end
                        else begin
                            ct_data_tmp_reg[j] = 0;
                        end
                ...
                if(image_mem_ct[k * four_byte_num  + i]!==ct_data_tmp_reg) begin
                ct_diff_count = ct_diff_count + 1;
                end
            end
        end
...
initial begin : write_ct
    integer write_ct_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_ct_count = 0; check_ct_count < ct_OPERATE_DEPTH; check_ct_count = check_ct_count + 1) begin
            wait (ongoing_process_number === process_num && process_busy === 0);
            get_ack = 1;
            if (write_ct_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write ct data 
                for (i = 0; i < four_byte_num; i = i + 1) begin
                    if (ct_c_bitwidth < 32) begin
                        ct_data_tmp_reg = mem_ct[check_ct_count];
                    end
                    else begin
                        for (j = 0; j < 32; j = j + 1) begin
                            if (i*32 + j < ct_c_bitwidth) begin
                                ct_data_tmp_reg[j] = mem_ct[check_ct_count][i*32 + j];
                            end
                            else begin
                                ct_data_tmp_reg[j] = 0;
                            end
                    ...
                    if(image_mem_ct[check_ct_count * four_byte_num  + i]!==ct_data_tmp_reg) begin
                        image_mem_ct[check_ct_count * four_byte_num + i]=ct_data_tmp_reg;
                        write (ct_data_in_addr + check_ct_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, ct_data_tmp_reg, write_ct_resp);
                        write_one_ct_data_done <= 1;
                        @(posedge clk);
                        write_one_ct_data_done <= 0;
                    end
                end
            end
            process_busy = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_pk_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (pk_c_bitwidth, pk_DEPTH, pk_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_pk_run_flag <= 1; 
        end
        else if ((write_one_pk_data_done == 1 && write_pk_count == pk_diff_count - 1) || pk_diff_count == 0) begin
            write_pk_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_pk_data_done === 1) begin
            write_pk_count = write_pk_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        pk_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            pk_write_data_finish <= 0;
        end
        if (write_pk_run_flag == 1 && write_pk_count == pk_diff_count) begin
            pk_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_pk
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    integer k;
    reg [31 : 0] pk_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = pk_c_bitwidth;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num);
    ceil_align_to_pow_of_two_four_byte_num = ceil_align_to_pow_of_two(four_byte_num);
    while (1) begin
        wait (AESL_ready_reg === 1);
        ...
        for (k = 0; k < pk_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (pk_c_bitwidth < 32) begin
                    pk_data_tmp_reg = mem_pk[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < pk_c_bitwidth) begin
                            pk_data_tmp_reg[j] = mem_pk[k][i*32 + j];
                        end
                        else begin
                            pk_data_tmp_reg[j] = 0;
                        end
                ...
                if(image_mem_pk[k * four_byte_num  + i]!==pk_data_tmp_reg) begin
                pk_diff_count = pk_diff_count + 1;
                end
            end
        end
...
initial begin : write_pk
    integer write_pk_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_pk_count = 0; check_pk_count < pk_OPERATE_DEPTH; check_pk_count = check_pk_count + 1) begin
            wait (ongoing_process_number === process_num && process_busy === 0);
            get_ack = 1;
            if (write_pk_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write pk data 
                for (i = 0; i < four_byte_num; i = i + 1) begin
                    if (pk_c_bitwidth < 32) begin
                        pk_data_tmp_reg = mem_pk[check_pk_count];
                    end
                    else begin
                        for (j = 0; j < 32; j = j + 1) begin
                            if (i*32 + j < pk_c_bitwidth) begin
                                pk_data_tmp_reg[j] = mem_pk[check_pk_count][i*32 + j];
                            end
                            else begin
                                pk_data_tmp_reg[j] = 0;
                            end
                    ...
                    if(image_mem_pk[check_pk_count * four_byte_num  + i]!==pk_data_tmp_reg) begin
                        image_mem_pk[check_pk_count * four_byte_num + i]=pk_data_tmp_reg;
                        write (pk_data_in_addr + check_pk_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, pk_data_tmp_reg, write_pk_resp);
                        write_one_pk_data_done <= 1;
                        @(posedge clk);
                        write_one_pk_data_done <= 0;
                    end
                end
            end
            process_busy = 0;
...
// Read data from file 
initial begin : read_ct_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [ct_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (ct_c_bitwidth , factor);
  fp = $fopen(`TV_IN_ct ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_ct); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < ct_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
...
// Read data from file 
initial begin : read_pk_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [pk_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (pk_c_bitwidth , factor);
  fp = $fopen(`TV_IN_pk ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_pk); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < pk_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
