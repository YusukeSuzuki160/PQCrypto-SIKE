...
typedef struct {
    ...
    XSikep503_kem_enc_hw_uio_map maps[ MAX_UIO_MAPS ];
} XSikep503_kem_enc_hw_uio_info;

/***************** Variable Definitions **************************************/
static XSikep503_kem_enc_hw_uio_info uio_info;
...
static int uio_info_read_name(XSikep503_kem_enc_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSikep503_kem_enc_hw_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSikep503_kem_enc_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSikep503_kem_enc_hw_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSikep503_kem_enc_hw_Initialize(XSikep503_kem_enc_hw *InstancePtr, const char* InstanceName) {
	XSikep503_kem_enc_hw_uio_info *InfoPtr = &uio_info;
