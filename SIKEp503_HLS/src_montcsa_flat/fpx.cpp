...
#pragma HLS DATAFLOW off
  #pragma HLS INLINE off
  // リソース重視オプション: FIOS-CSA-True(単体最速だがLUTが約4倍)ではなく
  // FIOS-CSA-Flat(通常の+演算子でFPGAの高速キャリーチェーンにマッピングされ
  // LUTを抑えつつ、元のmp_mul+rdc_montより確実に高速。かつ外側ループ完全展開
  // 設計のため元々レイテンシが決定的)を使用する。
  // N=8単体合成実測: 27cyc固定@7.295ns=197ns, DSP=250, LUT=42,388
  // (FIOS-CSA-True比 DSP -37%, LUT -75%)。
