...
{ // Computes [2^e](X:Z) on Montgomery curve with projective constant via e repeated doublings.
  ...
  // 固定回数ループ(最大503回)化に加え、反復ごとのコストも定数時間化。
  // 元は for(i=0;i<e;i++) という実行時可変トリップ数ループで、HLS が
  // レイテンシを静的に確定できず合成レポートが不定("?")になっていた。
  // トリップ数をMAX_Eに固定しただけでは、本体を「実行/スキップ」する
  // if(i<e)自体が実行時データ(ストラテジーテーブル由来のe)に依存する
  // ため、反復あたりのコストが依然として可変になり、合成レイテンシが
  // 広い範囲(min〜max)のままだった。ここでは毎回必ず1回の倍加を計算し
  // (スキップしない)、結果を採用するかどうかだけをビットマスクで選択
  // する(swap_pointsと同じ定数時間の手法)。これにより反復あたりのコス
  // トも実行時データに依存しなくなり、xDBLe全体のレイテンシが単一の
  // 固定値に収束する(引き換えに常にMAX_E回分の倍加コストを払う)。
  constexpr int MAX_E = 503;
  for (i = 0; i < MAX_E; i++)
  {
    point_proj_t Qnew = {0};
    xDBL(Q, Qnew, A24plus, C24);
    digit_t mask = 0 - (digit_t)(i < e);
    select_point(Q, Qnew, mask);
  }
}
...
{ // Computes [3^e](X:Z) on Montgomery curve with projective constant via e repeated triplings.
  ...
  // xDBLe と同じ理由・同じ手法(常に計算+マスク選択)で完全に定数時間化。
  constexpr int MAX_E = 503;
  for (i = 0; i < MAX_E; i++)
  {
    point_proj_t Qnew = {0};
    xTPL(Q, Qnew, A24minus, A24plus);
    digit_t mask = 0 - (digit_t)(i < e);
    select_point(Q, Qnew, mask);
  }
}
...
{ // 定数時間セレクト: mask==全1(0xFF...FF)ならP<-Q、mask==0ならPは不変。
  // xDBLe/xTPLe/eval_4_isog等で「実行時値に応じて計算をスキップする」
  // 代わりに「常に計算し、結果を採用するかどうかだけをマスクで選ぶ」
  // ようにするために用いる(定数時間暗号の標準手法)。分岐に依存しない
  // ため、HLSから見てもループの各反復が常に同じコストになり、合成後の
  // レイテンシが実行時データに依存しない単一の固定値へ収束する。
  unsigned int i;

  for (i = 0; i < NWORDS_FIELD; i++)
  {
    P->X[0][i] = (Q->X[0][i] & mask) | (P->X[0][i] & ~mask);
    P->X[1][i] = (Q->X[1][i] & mask) | (P->X[1][i] & ~mask);
    P->Z[0][i] = (Q->Z[0][i] & mask) | (P->Z[0][i] & ~mask);
    P->Z[1][i] = (Q->Z[1][i] & mask) | (P->Z[1][i] & ~mask);
  }
}

void copy_point(const point_proj_t P, point_proj_t Q)
{ // point_proj_t のコピー。以前は copy_words((digit_t*)P,(digit_t*)Q,32) の
  // ようにポインタキャストでフラット配列扱いしていたが、これは構造体の
  // 最初のフィールド(X)の宣言サイズを超えてインデックスアクセスする、
  // 型安全でないコードだった(実測でHLSの境界チェッカーが
  // "illegal out-of-bounds access"を検出した)。fpcopy を各フィールドへ
  // 個別に適用することで、型安全かつ既存の実績あるループパターンのみを
  // 使う。
  fpcopy(P->X[0], Q->X[0]);
  fpcopy(P->X[1], Q->X[1]);
  fpcopy(P->Z[0], Q->Z[0]);
...
// nbits を AliceOrBob(実行時引数)で分岐させると、HLSからは
// "実行時に0〜MAX_NBITSの任意値を取り得る変数"として扱われ、ループのレイ
// テンシが不定("?"→広大なmin/max幅)になっていた。#pragma HLS INLINE
// だけでは、Vitis HLS自身がループ本体を別ハードウェアブロックへ自動
// outlineしてしまい、その際にnbitsの値域(実際は250か253の2択のみ)が
// 再び「任意のint」へ抽象化され、定数畳み込みの効果が失われることを
// 実測で確認した。そこで nbits を実行時変数ではなく C++テンプレート
// 引数(コンパイル時定数)にし、HLSの outline/インライン判断に依存せず
// ループ境界そのものを合成前に確定させる。
template <int NBITS>
static void LADDER3PT_impl(const f2elm_t xP, const f2elm_t xQ, const f2elm_t xPQ, const digit_t *m, point_proj_t R, const f2elm_t A)
{
  ...
  // Main loop: NBITS はテンプレート引数(コンパイル時定数)なので、
  // ループ境界は合成前に確定しており、実行時可変性は一切残らない。
  for (i = 0; i < NBITS; i++)
  {
...
#pragma HLS INLINE
  if (AliceOrBob == ALICE)
  {
    LADDER3PT_impl<OALICE_BITS>(xP, xQ, xPQ, m, R, A);
  }
  else
  {
    LADDER3PT_impl<OBOB_BITS>(xP, xQ, xPQ, m, R, A);
  }
}