...
// mp_mul は本プロジェクトでは fp2mul_mont から nwords==NWORDS_FIELD(=8)
// のみで3回呼ばれる(GF(p^2)のKaratsuba風乗算で未還元の生の積を得るため。
// fpmul_mont/fpsqr_mont(乗算と還元が段ごとに融合したFIOS方式)は還元まで
// 行ってしまうため代用できない)。
//
// 以前の実装(mpx::PackedOps: オペランドをap_uint<512>へ丸ごとpackして
// 1回の大きな乗算を行いunpackする方式)はSIKE全体合成でDSP/LUTを大きく
// 消費する主因の一つだった。fp2mul_montが同型写像ウォーク中の支配的な
// 乗算経路であるため、ここをFIOS-CSA-Flatと同じ「ネイティブ幅倍型での
// 乗算+通常の+によるキャリー伝播」というリソース効率の良いスタイルの
// スクールブック乗算(mont_raw_mul::RawMul、還元なし)へ置き換える。
// nwordsは実行時引数のままだが、LADDER3PT/copy_wordsと同じ理由により
// 呼び出し側が定数であってもHLSからは可変長ループに見えかねないため、
// 唯一の実際の呼び出しパターン(nwords==NWORDS_FIELD)に対してテンプレート
// 引数(コンパイル時定数)で直接呼び出す形にする。
void mp_mul(const digit_t *a, const digit_t *b, digit_t *c, const unsigned int /*nwords*/)
{
#pragma HLS INLINE off
    mont_raw_mul::RawMul<digit_t, NWORDS_FIELD>::mul(a, b, c);
}
