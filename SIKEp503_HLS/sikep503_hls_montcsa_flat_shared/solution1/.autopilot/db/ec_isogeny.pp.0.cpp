# 1 "src_montcsa_flat/ec_isogeny.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "src_montcsa_flat/ec_isogeny.cpp" 2
...
# 8 "src_montcsa_flat/ec_isogeny.cpp" 2
...
void xDBLe(const point_proj_t P, point_proj_t Q, const f2elm_t A24plus, const f2elm_t C24, const int e)
{
  ...
  copy_point(P, Q);
# 46 "src_montcsa_flat/ec_isogeny.cpp"
  constexpr int MAX_E = 503;
  VITIS_LOOP_47_1: for (i = 0; i < MAX_E; i++)
  {
    point_proj_t Qnew = {0};
...
void xTPLe(const point_proj_t P, point_proj_t Q, const f2elm_t A24minus, const f2elm_t A24plus, const int e)
{
  ...
  constexpr int MAX_E = 503;
  VITIS_LOOP_136_1: for (i = 0; i < MAX_E; i++)
  {
    point_proj_t Qnew = {0};
...
void swap_points(point_proj_t P, point_proj_t Q, const digit_t option)
{
  ...
  VITIS_LOOP_287_1: for (i = 0; i < 8; i++)
  {
...
void select_point(point_proj_t P, const point_proj_t Q, const digit_t mask)
{
  ...
  VITIS_LOOP_313_1: for (i = 0; i < 8; i++)
  {
...
void copy_point(const point_proj_t P, point_proj_t Q)
{
  ...
  fpcopy503(P->X[0], Q->X[0]);
  fpcopy503(P->X[1], Q->X[1]);
  fpcopy503(P->Z[0], Q->Z[0]);
  fpcopy503(P->Z[1], Q->Z[1]);
}
# 345 "src_montcsa_flat/ec_isogeny.cpp"
template <int NBITS>
static void LADDER3PT_impl(const f2elm_t xP, const f2elm_t xQ, const f2elm_t xPQ, const digit_t *m, point_proj_t R, const f2elm_t A)
{
  ...
  VITIS_LOOP_371_1: for (i = 0; i < NBITS; i++)
  {
    bit = (m[i >> 6] >> (i & (64 - 1))) & 1;
    swap = bit ^ prevbit;
    prevbit = bit;
...
#pragma HLS INLINE
 if (AliceOrBob == 0)
  {
    LADDER3PT_impl<250>(xP, xQ, xPQ, m, R, A);
  }
  else
  {
    LADDER3PT_impl<253>(xP, xQ, xPQ, m, R, A);
  }
}
